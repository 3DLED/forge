...
import type { PersonalRecord } from '../../domain/training';
...
export function prMarks(record: PersonalRecord, units: UnitSystem): PrMark[] {
  ...
  if (record.bestReps) {
    marks.push({ label: 'Most reps in a set', value: String(record.bestReps) });
  }
  ...
  if (!record.bestRounds && record.bestTimeSec) {
    marks.push({ label: 'Longest hold', value: formatDuration(record.bestTimeSec) });
  }

  if (record.bestDistanceM) {
    marks.push({ label: 'Furthest', value: formatDistance(record.bestDistanceM, units) });
  }
  ...
  return (
    <Sheet title={name} onClose={onClose}>
      <p className="small muted">
        Set {formatDayLabel(record.date).toLowerCase()}.
      </p>
